...
def _ensure_monitored_jobs_schema() -> None:
    ...
    try:
        ...
        for stmt in (
            ...
            "ALTER TABLE IF EXISTS job_screen_questions ADD COLUMN IF NOT EXISTS question_type TEXT",
...
async def get_job_outreach_stats(job_id_or_ref: str, user: UserIdentity = Depends(get_current_user)):
    ...
    try:
        with conn.cursor() as cur:
            ...
            result = cur.fetchone()
            if not result:
                return dict(_EMPTY_OUTREACH_STATS)
