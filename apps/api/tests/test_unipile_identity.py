...
def test_extract_profile_data_extracts_name_fields():
    svc = UnifiedCandidateSearch()
    
    # Full name provided
    ext1 = svc._extract_linkedin_profile_data({"full_name": "Jane Doe"})
    assert ext1["_profile_name"] == "Jane Doe"
    assert ext1["_profile_first_name"] == "Jane"
    assert ext1["_profile_last_name"] == "Doe"

    # Only first/last provided
    ext2 = svc._extract_linkedin_profile_data({"first_name": "John", "lastName": "Smith"})
    assert ext2["_profile_name"] == "John Smith"
    assert ext2["_profile_first_name"] == "John"
    assert ext2["_profile_last_name"] == "Smith"


def test_unified_search_name_correction_logic():
    # Directly test the correction logic block that lives in UnifiedCandidateSearch
    # without mocking the entire search flow.
    from services.unified_candidate_search import _UNIPILE_GENERIC_PROFILE_NAMES
    
    def apply_correction(cand_name: str, profile_name: str, profile_first: str, profile_last: str) -> dict:
        cand = {"name": cand_name, "firstName": "OldFirst", "lastName": "OldLast"}
        profile_data = {
            "_profile_name": profile_name,
            "_profile_first_name": profile_first,
            "_profile_last_name": profile_last,
        }
        
        # The exact logic from unified_candidate_search.py
        _p_name = str(profile_data.get("_profile_name") or "").strip()
        try:
            if _p_name:
                _current = str(cand.get("name") or "")
                _is_gen = _p_name.lower() in _UNIPILE_GENERIC_PROFILE_NAMES
                _is_fallback = (
                    not _current
                    or "|" in _current
                    or _current.lower().startswith("linkedin professional ")
                    or _current.lower().startswith("linkedin candidate")
                    or _current.lower() in _UNIPILE_GENERIC_PROFILE_NAMES
                )
                if not _is_gen and _is_fallback:
                    cand["name"] = _p_name
    ...
    # 1. Headline fallback -> overwritten
    res = apply_correction("Software Engineer | Builder", "Jane Doe", "Jane", "Doe")
    assert res["name"] == "Jane Doe"
    assert res["firstName"] == "Jane"
    
    # 2. LinkedIn Professional fallback -> overwritten
    res = apply_correction("LinkedIn Professional 123456", "John Smith", "John", "Smith")
    assert res["name"] == "John Smith"
    
    # 3. Empty name -> overwritten
    res = apply_correction("", "Alice", "Alice", "")
    assert res["name"] == "Alice"
    
    # 4. Profile name is generic -> skip overwrite
    res = apply_correction("Engineer | Tech", "LinkedIn Member", "LinkedIn", "Member")
    assert res["name"] == "Engineer | Tech"  # Kept the fallback
    
    # 5. Current name is already good (not a fallback) -> skip overwrite
    res = apply_correction("Julia Rogers", "Julia Rogers", "Julia", "Rogers")
    assert res["name"] == "Julia Rogers"
    
    # 6. Current name is good, but differs (e.g. nickname) -> skip overwrite
    res = apply_correction("J. Rogers", "Julia Rogers", "Julia", "Rogers")
    assert res["name"] == "J. Rogers"
