    ...
    try {
      ...
      const jobs: Job[] = Object.entries(data.jobs).map(([id, details]: [string, any]) => {
        ...
        return {
          ...
          candidatesSourced: details.candidatesSourced || Math.floor(Math.random() * 200), // Placeholder data for visualization
          resumesShortlisted: details.resumesShortlisted || 0,
          completeSubmissions: details.completeSubmissions || 0,
          passSubmissions: details.passSubmissions || 0,
          pairExternalSubs: details.pairExternalSubs || 0,
          feedbackCompleted: details.feedbackCompleted || 0,
          timeToFirstPass: details.timeToFirstPass || 0,
        };
      });
    ...
    <div className="space-y-6 max-w-[1200px] mx-auto pb-10">
      ...
      <div className="flex justify-between items-center gap-4 mt-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button asChild className="flex items-center gap-2 h-10 px-5 bg-[#4f46e5] hover:bg-[#4338ca] text-white font-semibold text-[13px] rounded-lg shadow-sm transition-all active:scale-95">
            <Link href="/jobs/new">
      ...
      <div className="bg-white rounded-2xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] border border-slate-200 overflow-hidden mt-2">
          ...
          <table className="min-w-full divide-y divide-slate-100">
            <thead className="bg-[#fcfdfd]">
              <tr>
                ...
                <SortableHeader field="customer">CUSTOMER NAME</SortableHeader>
                ...
                <SortableHeader field="timeToFirstPass">TIME TO FIRST</SortableHeader>
        ...
        {filteredJobs.length === 0 && (
          <div className="text-center py-16">
            <p className="text-[14px] font-medium text-slate-500">No jobs found matching your search criteria.</p>
          </div>
        )}
      </div>
    </div>
