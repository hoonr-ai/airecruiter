...
# Fix 1: Move import to top
content = content.replace("from routers.launch_report import build_merged_outreach_payload\n", "")
if "from routers.launch_report import build_merged_outreach_payload" not in content:
    content = content.replace("from fastapi import APIRouter", "from routers.launch_report import build_merged_outreach_payload\nfrom fastapi import APIRouter")

# Fix 2: Helper _to_iso_z
helper_code = """
...
"""
if "def _to_iso_z" not in content:
    content = content.replace("router = APIRouter()", helper_code + "\nrouter = APIRouter()")

# Replace manual dt normalization in master pool
...
content = content.replace(old_dt1, new_dt1)

# And in get_job_candidates
old_dt2 = """            import datetime
            dt_val = cand.get("engage_created_at")
            if isinstance(dt_val, datetime.datetime):
                cand["engage_created_at"] = dt_val.isoformat() + "Z"
            elif isinstance(dt_val, str) and dt_val and not dt_val.endswith("Z"):
...
content = content.replace(old_dt3, new_dt3)

# Fix 3: SQL Error
content = content.replace("SELECT DISTINCT ON (lookup_id) lookup_id, title, screening_level", "SELECT DISTINCT ON (lookup_id) lookup_id, title, screening_level, recruiter_emails")

# Fix 4: Restore try...except for score
...
new_avg1 = """            try:
                ...
                cand["total_fit_score"] = None"""
content = content.replace(old_avg1, new_avg1)

# Same for the other get_job_candidates loop
# old_score1 might match multiple places! Let's just do it directly.
# Wait, I replaced all occurrences above, let's write to file and check!
